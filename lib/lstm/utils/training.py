...
def accuracy_calculation(original_seq,decoded_seq,ignore_value=0,isPrint = True):
    ...
    for i,origin_label in enumerate(original_seq):
        dec_label  = [j for j in decoded_seq[i] if j!=ignore_value]
        org_label = [l for l in origin_label if l!=ignore_value]
        if isPrint and i<cfg.VAL.PRINT_NUM:
            print('seq{0:4d}: origin: {1} \n         decoded:{2}'.format(i,org_label,dec_label))
        new_dec_label = [k for k in dec_label if k!=1]
        new_org_label = [n for n in org_label if n!=1]
        if new_org_label == new_dec_label: count+=1

        num_rt, lable_len, acc = compute_acc(new_org_label, new_dec_label)
        if isPrint and i<cfg.VAL.PRINT_NUM:
            print('acc:{:.3f}'.format(acc))
        total_rt = total_rt + num_rt
        total_len = total_len + lable_len
    acc_words = total_rt / total_len
    print('acc_words:{:.4f}'.format(acc_words))
...
def accuracy_calculation(original_seq,decoded_seq,ignore_value=0,isPrint = True):
    ...
    compute_acc(original_seq, decoded_seq)
    return count*1.0/len(original_seq)
'''
