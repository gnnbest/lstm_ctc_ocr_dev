...
def write_image_annotation_pairs_to_tfrecord(img_path, tfrecords_filename):
    ...
    for root,subfolder,fileList in os.walk(img_path):
        for fname in fileList:
            dir_fname = os.path.join(root,fname)
            img = np.array(Image.open(dir_fname))
            #code = re.match(r'.*\/[0-9]+_(.*)(_1)?\..*', fname).group(1)
            code = fname.rsplit('.',1)[0].rsplit('_',1)[1]
...
if __name__=='__main__':
    wrtie_test(img_path='../../utils/data/train',tfrecords_filename='../../utils/data/train.tfrecords')
